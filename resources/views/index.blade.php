      ...
      <div class="flex items-center gap-5 overflow-x-hidden" id="carousel">
        @if (count($products) > 0)
          @foreach ($products as $product)
            <a href="{{ route('product.product-detail.show', $product['slug']) }}" class="block shrink-0">
              <div class="border border-black relative">
                <img class="bg-cover object-cover aspect-[4/5] w-80 overflow-hidden" src={{ $product['image'] }}
              ...
              <h1 class="capitalize font-semibold text-xl mt-1 text-nowrap overflow-hidden text-ellipsis font-[Inter]">
                {{ $product['name'] }}</h1>
